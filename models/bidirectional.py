...
class Model:
    def __init__(
        ...
        self.backward_hidden_layer = tf.compat.v1.placeholder(
            tf.compat.v1.float32, shape = (None, num_layers * size_layer)
        )
        ...
        with tf.compat.v1.variable_scope('decoder', reuse = False):
            ...
            drop_backward_decoder = tf.rnn.DropoutWrapper(
            backward_rnn_cells_decoder, output_keep_prob = forget_bias
            )
            forward_backward_decoder = tf.rnn.DropoutWrapper(
        ...
        self.outputs = tf.compat.v1.concat(self.outputs, 2)
